...
const ModelArch = @import("constants.zig").ModelArch;
const TensorNames = @import("constants.zig").TensorNames;
const ModelTensors = @import("constants.zig").ModelTensorsByArch;

fn defineMappings(allocator: std.mem.Allocator) !TensorNameMap.MappingsConfig {
    var map = TensorNameMap.MappingsConfig.init(allocator);
    ...
    try map.put(ModelTensor.ROPE_FREQS, &[_][]const u8{
...
fn defineBlockMappings(allocator: std.mem.Allocator) !TensorNameMap.BlockMappingsConfig {
    var block_map = TensorNameMap.BlockMappingsConfig.init(allocator);
...
pub const TensorNameMap = struct {
    pub const MappingsConfig = std.AutoHashMap(ModelTensor, []const []const u8);
    pub const BlockMappingsConfig = std.AutoHashMap(ModelTensor, []const []const u8);
    pub const Mappings = std.StringArrayHashMap(MappingEntry);

    const Self = @This();

    mappings: Mappings,
    mappings_cfg: MappingsConfig,
    block_mappings_cfg: BlockMappingsConfig,

    pub fn init(allocator: std.mem.Allocator, arch: ModelArch, n_blocks: u32) !TensorNameMap {
        const mappings_cfg = try defineMappings(allocator);
        const block_mappings_cfg = try defineBlockMappings(allocator);

        var map = Mappings.init(allocator);

        // 1. Process standard mappings
        var iter = mappings_cfg.iterator();
        while (iter.next()) |entry| {
            const tensor = entry.key_ptr.*;
            const keys = entry.value_ptr.*;
            if (!hasTensor(ModelTensors.get(arch), tensor)) continue;

            const name = TensorNames.get(tensor);
            try map.put(name, .{ .tensor = tensor, .name = name });

            for (keys) |key| {
                try map.put(key, .{ .tensor = tensor, .name = name });
            }
        }

        // 2. Process block mappings
        for (0..n_blocks) |bid| {
            var biter = block_mappings_cfg.iterator();
            ...
            while (biter.next()) |entry| {
                ...
                const template = TensorNames.get(tensor);
                ...
                const pieces_array = [_][]const u8{ prefix, bid_str, suffix };
                var temp_wrapped: [3][]const u8 = undefined;

                // Wrap each element in a slice of slices with one element
                inline for (0.., pieces_array) |i, piece| {
                    // Create a one-element array to wrap piece
                    temp_wrapped[i] = piece;
                }

                const tensor_name = try std.mem.concat(allocator, u8, &temp_wrapped);
                ...
                for (keys) |key| {
                    ...
                    const pieces_arr = [_][]const u8{ pref, bid_str, suff };
                    var temp_wrap: [3][]const u8 = undefined;
                    inline for (0.., pieces_arr) |i, piece| {
                        // Create a one-element array to wrap piece
                        temp_wrap[i] = piece;
                    }
                    const t_name = try std.mem.concat(allocator, u8, &temp_wrap);
                    //defer allocator.free(t_name);
                    try map.put(t_name, .{ .tensor = tensor, .name = tensor_name });
                }
            }
        }

        return TensorNameMap{
            .mappings = map,
            .mappings_cfg = mappings_cfg,
            .block_mappings_cfg = block_mappings_cfg,
        };
    }
    /// returns whether `actual` matches `template` with `{}` wildcard
    fn match_template(actual: []const u8, template: []const u8) bool {
        const parts = std.mem.split(u8, template, "{}");
        if (parts.len != 2) return false; // assume one `{}`

        return std.mem.startsWith(u8, actual, parts[0]) and
            std.mem.endsWith(u8, actual, parts[1]);
    }
    pub fn get_type_and_name(
        self: *Self,
        ...
        // 2. Try suffix trimming logic
        for (try_suffixes) |suffix| {
            if (std.mem.endsWith(u8, key, suffix)) {
                const trimmed = key[0 .. key.len - suffix.len];
                if (self.mappings.get(trimmed)) |entry| {
                    const name = try std.fmt.allocPrint(allocator, "{s}{s}", .{ entry.name, suffix });
                    return MappingEntry{ .name = name, .tensor = entry.tensor };
                }
            }
        }

        // 3. Try template pattern match
        var it = self.mappings.iterator();
        while (it.next()) |entry| {
            const candidate = entry.key_ptr.*;
            const value = entry.value_ptr.*;

            // Try to extract dynamic parts
            // Examples:
            // candidate = "model.layers.0.mlp.down_proj"
            // value.name = "blk.0.ffn_down"

            const tensor_template = TensorNames.get(value.tensor);
            if (std.mem.indexOf(u8, tensor_template, "{}")) |brace_index| {
                const prefix = tensor_template[0..brace_index];
                const suffix = tensor_template[brace_index + 2 ..];

                if (std.mem.startsWith(u8, candidate, prefix) and std.mem.endsWith(u8, candidate, suffix)) {
                    //const candidate_middle = candidate[prefix.len .. candidate.len - suffix.len];
                    const key_middle = key[prefix.len .. key.len - suffix.len];

                    if (isAllDigits(key_middle) and std.mem.eql(u8, key, candidate)) {
                        return MappingEntry{ .tensor = value.tensor, .name = value.name };
                    }
                }
            }
        }
    ...
    pub fn get_type(self: *Self, allocator: std.mem.Allocator, key: []const u8, try_suffixes: []const []const u8) ?ModelTensor {
        const result = try self.get_type_and_name(allocator, key, try_suffixes) orelse return null;
        return result.tensor;
    }

    pub fn contains(self: *Self, key: []const u8) bool {
        return self.mappings.contains(key);
    }

    pub fn get(self: *Self, key: []const u8) ![]const u8 {
        const entry = self.mappings.get(key) orelse return error.NotFound;
        return entry.name;
    }
    ...
    pub fn getFormattedBlockMappings(self: *Self, allocator: std.mem.Allocator, tensor: ModelTensor, bid: usize) ![]const []const u8 {
        const templates = self.block_mappings_cfg.get(tensor) orelse return &[_][]const u8{};
...
fn hasTensor(tensors: []const ModelTensor, tensor: ModelTensor) bool {
    for (tensors) |t| {
...
pub fn getTensorNameMap(allocator: std.mem.Allocator, arch: ModelArch, n_blocks: u32) !TensorNameMap {
    return TensorNameMap.init(allocator, arch, n_blocks);
}

fn format_runtime(template: []const u8, bid: usize, allocator: std.mem.Allocator) ![]u8 {
    const parts = try std.mem.split(allocator, template, "{}");
    defer allocator.free(parts);

    const bid_str = try std.fmt.allocPrint(allocator, "{}", .{bid});
    return std.mem.concat(allocator, []const []const u8{ parts[0], bid_str, parts[1] });
}
