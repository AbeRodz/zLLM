...
pub const Metadata = struct {
    ...
    pub fn init(
        ...
        meta_opt: ?std.StringHashMap(Value),
        ...
        for (tensor_entries, 0..) |entry, i| {
            try index_map.put(entry.name, i);
            try tensors.append(entry.info);
        }

        const metadata = Metadata{
            .metadata = meta_opt orelse std.StringHashMap(Value).init(allocator),
            .tensors = tensors,
            .index_map = index_map,
        };
    ...
    pub fn put(self: *Metadata, allocator: std.mem.Allocator, model_name: []const u8, key_suffix: []const u8, value: Value) !void {
        const full_key_tmp = try std.fmt.allocPrint(allocator, "{s}.{s}", .{ model_name, key_suffix });
    ...
    pub fn offsetKeys(self: *Metadata, allocator: std.mem.Allocator) ![][]const u8 {
        var kvs = try allocator.alloc(struct { name: []const u8, index: usize }, self.index_map.count());

        var i: usize = 0;
        var iter = self.index_map.iterator();
        while (iter.next()) |entry| {
            kvs[i] = .{ .name = entry.key_ptr.*, .index = entry.value_ptr.* };
            i += 1;
        }

        std.sort.sort(kvs, {}, struct {
            pub fn lessThan(_: void, a: @TypeOf(kvs[0]), b: @TypeOf(kvs[0])) bool {
...
pub fn parseSafetensorsFromBuffer(allocator: std.mem.Allocator, buffer: []const u8) !Metadata {
    ...
    var meta_map = std.StringHashMap(Value).init(allocator);
    ...
    const metadata = try Metadata.init(allocator, meta_map, tensor_entries.items);
...
pub fn read(model_name: []const u8, allocator: std.mem.Allocator) !void {
    ...
    _ = try parseSafetensorsFromBuffer(allocator, buffer);
}
